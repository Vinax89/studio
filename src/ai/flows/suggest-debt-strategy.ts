...
export type SuggestDebtStrategyInput = z.infer<
  typeof SuggestDebtStrategyInputSchema
>;

const SuggestDebtStrategyOutputSchema = z.object({
  recommendedStrategy: z
    .enum(["avalanche", "snowball"])
    .describe("The recommended strategy, either 'avalanche' or 'snowball'."),
  strategyReasoning: z
    .string()
    .describe("The reasoning behind the recommended strategy."),
  payoffOrder: z
    .array(
      z.object({
        debtName: z.string(),
    ...
    .describe(
      "The recommended order to pay off the debts, starting with priority 1.",
    ),
  summary: z.string().describe("A brief, encouraging summary of the plan."),
});
export type SuggestDebtStrategyOutput = z.infer<
  typeof SuggestDebtStrategyOutputSchema
>;
