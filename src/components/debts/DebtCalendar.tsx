...
export default function DebtCalendar({
  ...
  const grid = useMemo(
    () => monthMatrix(cursor.getFullYear(), cursor.getMonth(), startOn),
    [cursor, startOn],
  );
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (
        (isCmd && e.metaKey && e.key.toLowerCase() === "k") ||
        (!isCmd && e.ctrlKey && e.key.toLowerCase() === "k")
      ) {
        e.preventDefault();
    ...
    <div className="w-full max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <div className="text-xl font-semibold select-none min-w-[10ch] text-center">
            {headerLabel}
          </div>
          <Button
            variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        ...
        <Stat
          label="Scheduled (Autopay)"
          value={currency(monthTotals.autopay)}
        />
      ...
      <div className="grid grid-cols-7 text-center text-sm font-medium text-muted-foreground">
        {Array.from({ length: 7 }).map((_, i) => (
          <div key={i} className="py-2 select-none">
            {WEEKDAYS[(i + startOn) % 7]}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-1 rounded-lg bg-muted/50 p-1">
        {grid.map((date, idx) => {
          ...
          const sumForDay = dayEvents.reduce(
            (s, e) => s + e.debt.minimumPayment,
            0,
          );
          ...
          return (
            <div
              ...
              onClick={(e) => {
                if (
                  (e.target as HTMLElement).closest("[data-chip]") ||
              ...
              <div className="mt-2 flex flex-col gap-1">
                {dayEvents.map(({ debt }) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={"truncate " + (paid ? "line-through" : "")}
                      >
                        {debt.name}
                      </span>
                      <span
                        className={
                          ...
                          (paid ? "line-through" : "font-semibold")
                        }
                      >
                        {currency(debt.minimumPayment)}
                      </span>
                      {debt.autopay && (
                        <span className="text-[10px] px-1 py-0.5 rounded bg-black/10">
                          AUTO
                        </span>
                      )}
                    </div>
      ...
      {showForm && (
        <DebtForm
          ...
          onDelete={
            activeDebt
              ? () => {
                  deleteDebt(activeDebt.id);
                  setShowForm(false);
                }
              : undefined
          }
          onSave={(values) => {
            const next: Debt = activeDebt
              ? { ...activeDebt, ...values }
          ...
          onMarkPaid={(dateISO) =>
            activeDebt && markPaid(dateISO, activeDebt.id)
          }
          onUnmarkPaid={(dateISO) =>
            activeDebt && unmarkPaid(dateISO, activeDebt.id)
          }
        />
      )}
    </div>
...
function DebtForm({
  ...
  const [initialAmount, setInitialAmount] = useState<string>(
    initial ? String(initial.initialAmount) : "",
  );
  const [currentAmount, setCurrentAmount] = useState<string>(
    initial ? String(initial.currentAmount) : "",
  );
  ...
  function handleSave() {
    ...
    if (
      !name.trim() ||
      [initAmt, currAmt, intRate, minPay].some(isNaN) ||
      minPay <= 0
    ) {
  ...
  return (
    ...
    >
      <div
        ref={ref}
        tabIndex={-1}
        className="w-full max-w-lg rounded-2xl bg-background p-6 shadow-2xl outline-none"
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold">
            {initial ? "Edit" : "Add"} Debt
          </h2>
          <Button
            variant="ghost"
            size="icon"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormLabel label="Name">
            <Input
              ...
              placeholder="e.g., X1 Card"
            />
          </FormLabel>
          <FormLabel label="Interest Rate (%)">
            <Input
              value={interestRate}
          ...
          <FormLabel label="Initial Amount ($)">
            <Input
              value={initialAmount}
          ...
          <FormLabel label="Current Amount ($)">
            <Input
              value={currentAmount}
          ...
          <FormLabel label="Minimum Payment ($)">
            <Input
          ...
          <FormLabel label="Anchor Due Date">
            <Input
              type="date"
              value={dueDate}
          ...
          <FormLabel label="Chip Color (optional)">
            <Input
              type="color"
              value={color}
        ...
        {initial && (
          <div className="mt-4 p-3 rounded-xl bg-muted/80 border flex items-center gap-2">
            <span className="text-sm font-medium">
              Status for <strong>{dateISO}</strong>:
            </span>
            {!paidToday ? (
              <Button size="sm" onClick={() => onMarkPaid(dateISO)}>
                Mark Paid
              </Button>
            ) : (
              <Button
        ...
        <div className="mt-6 flex items-center justify-between">
          <div className="text-xs text-muted-foreground">
            Tip: Press <kbd className="kbd">Ctrl/Cmd+K</kbd> to quick‑add
          </div>
          <div className="flex items-center gap-2">
            {onDelete && (
...
function Toggle({
  ...
  return (
    ...
    >
      <span
        className={
          "absolute top-0.5 transition-transform duration-200 ease-in-out " +
