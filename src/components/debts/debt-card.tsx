...
export function DebtCard({ debt, onDelete, onUpdate }: DebtCardProps) {
  ...
  const progress =
    debt.initialAmount > 0
      ? (debt.currentAmount / debt.initialAmount) * 100
      : 0;
      ...
      <CardHeader>
        <div className="flex justify-between items-start">
          <div>
            ...
            <CardDescription>{debt.interestRate}% APR</CardDescription>
          </div>
          <span
      ...
      <CardContent className="space-y-2">
        ...
        <div className="text-sm text-muted-foreground">
          ...
          <p>
            ...
            is due on {displayDate}.
          </p>
        </div>
      </CardContent>
      <CardFooter className="flex justify-end gap-2">
        <Button variant="ghost" size="sm" onClick={() => onUpdate(debt)}>
          Edit
        </Button>
        <Button
          variant="destructive"
