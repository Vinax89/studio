...
export default function CashflowPage() {
  ...
  const [cashflowResult, setCashflowResult] =
    useState<CalculateCashflowOutput | null>(null);
  ...
  const [lastEnteredShift, setLastEnteredShift] = useState<ShiftDetails | null>(
    null,
  );
  ...
  const handleCashflowSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "There was an error calculating your cashflow. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleDateSelect = (date: Date | undefined) => {
    ...
    if (date) {
      ...
      if (existingShift) {
        ...
        setPremiumPay(
          existingShift.premiumPay ? String(existingShift.premiumPay) : "",
        );
        ...
        setPremiumPay(
          lastEnteredShift.premiumPay
            ? String(lastEnteredShift.premiumPay)
            : "",
        );
      ...
      <div className="lg:col-span-2 space-y-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">
            Monthly Cashflow & Shift Planner
          </h1>
          <p className="text-muted-foreground">
            Estimate your cashflow and project income from your shifts.
          </p>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Provide your annual income, estimated annual taxes, and total
              monthly deductions.
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="grid sm:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setEstimatedTaxes(e.target.value)}
                  />
                </div>
                <div className="space-y-2">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    $
                    {cashflowResult.grossMonthlyIncome.toLocaleString("en-US", {
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    $
                    {cashflowResult.netMonthlyIncome.toLocaleString("en-US", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Wallet className="h-5 w-5" /> Analysis
                </CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Select a date to highlight a pay period and add shifts.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="grid grid-cols-3 gap-4 text-center">
            <div className="space-y-1">
              ...
              <p className="text-lg font-bold">
                $
                {monthlyStats.totalMonthlyIncome.toLocaleString("en-US", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </p>
            ...
            <div className="space-y-1">
              ...
              <p className="text-lg font-bold">
                {monthlyStats.totalMonthlyHours.toFixed(1)}
              </p>
            </div>
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">Burnout Index</p>
              <p className="text-lg font-bold">
                {monthlyStats.burnoutIndex.toFixed(1)}% OT
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        {selectedDate && (
          <Card>
            <CardHeader>
              <CardTitle>
                Shift Details for {selectedDate.toLocaleDateString()}
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              {payPeriod.from && payPeriod.to && (
                <CardDescription>
                  {payPeriod.from.toLocaleDateString()} -{" "}
                  {payPeriod.to.toLocaleDateString()}
                </CardDescription>
              )}
            </CardHeader>
            <CardContent>
              {shiftsInPayPeriod.length > 0 ? (
                <div className="space-y-4">
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">
                        $
                        {payPeriodCalculation.totalIncome.toLocaleString(
                          "en-US",
                          {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                    ...
                    <CardContent>
                      <div className="text-sm space-y-1">
                        <div className="flex justify-between">
                          ...
                          <span>
                            {payPeriodCalculation.regularHours.toFixed(2)}
                          </span>
                        </div>
                        <div className="flex justify-between font-semibold text-primary">
                          <span>Overtime Hours:</span>{" "}
                          <span>
                            {payPeriodCalculation.overtimeHours.toFixed(2)}
                          </span>
                        </div>
                        <div className="flex justify-between font-bold">
                          <span>Total Hours:</span>{" "}
                          <span>
                            {payPeriodCalculation.totalHours.toFixed(2)}
                          </span>
                        </div>
                      </div>
                    </CardContent>
                ...
                <p className="text-sm text-muted-foreground">
                  No shifts added in this pay period yet.
                </p>
              )}
            </CardContent>
          </Card>
