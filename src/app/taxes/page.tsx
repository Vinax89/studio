  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!income || !state) {
      toast({
        ...
        description: "Please fill out income and state to estimate your taxes.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const result = await estimateTax({
        ...
        state,
        filingStatus,
        ...(localRate ? { localRate: parseFloat(localRate) } : {}),
      });
      setTaxResult(result);
    } catch (error) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Get an AI-powered estimate of your annual tax burden using 2025 brackets.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enter your income, filing status, and location to get a projection. This is not financial advice.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="city">City or ZIP</Label>
                <Input
                  id="city"
                  placeholder="e.g., Austin or 73301"
                  value={city}
                  onChange={(e) => setCity(e.target.value)}
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="local-rate">Local Tax Rate (%)</Label>
                <Input
                  id="local-rate"
                  type="number"
                  placeholder="e.g., 1.5"
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  ${taxResult.estimatedTax.toLocaleString()}
                </div>
                <p className="text-xs text-muted-foreground">
                  Total estimated federal, state, and local tax.
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  Your total tax as a percentage of your income.
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    <span>Federal</span>
                    <span>${taxResult.federalTax.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>State</span>
                    <span>${taxResult.stateTax.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Local</span>
                    <span>${taxResult.localTax.toLocaleString()}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <FileText className="h-5 w-5" /> Breakdown
              </CardTitle>
              <CardDescription>
                A detailed explanation of how the tax was estimated.
              </CardDescription>
            </CardHeader>
            <CardContent>
